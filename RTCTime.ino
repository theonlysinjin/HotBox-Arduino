...
#define DS1302_SCLK_PIN   12    // Arduino pin for the Serial Clock
#define DS1302_IO_PIN     13    // Arduino pin for the Data I/O
...
//  Serial.begin(9600);
  ...
  if (setit) {
    ...
    rtc.Seconds    = bin2bcd_l( toSetseconds);
    rtc.Seconds10  = bin2bcd_h( toSetseconds);
    rtc.CH         = 0;      // 1 for Clock Halt, 0 to run;
    rtc.Minutes    = bin2bcd_l( toSetminutes);
    rtc.Minutes10  = bin2bcd_h( toSetminutes);
    ...
    rtc.h24.Hour   = bin2bcd_l( toSethours);
    rtc.h24.Hour10 = bin2bcd_h( toSethours);
    rtc.h24.hour_12_24 = 0; // 0 for 24 hour format
    rtc.Date       = bin2bcd_l( toSetdayofmonth);
    rtc.Date10     = bin2bcd_h( toSetdayofmonth);
    rtc.Month      = bin2bcd_l( toSetmonth);
    rtc.Month10    = bin2bcd_h( toSetmonth);
    rtc.Day        = toSetdayofweek;
    rtc.Year       = bin2bcd_l( toSetyear - 2000);
    rtc.Year10     = bin2bcd_h( toSetyear - 2000);
    rtc.WP = 0;  
